        ...
        if len(pair) == 2 :
            ...
            if link != "NULL" and link != "":
                ...
                if value != "" and value !="\t" :
                    ...
                    # sys.stdout.flush()
                    # Adv Mode
                    f = open("stockAdv.txt","a") # Can be Global
                    if (not dictStore.has_key(info)) or dictStore[info] != pagesite :
                        dictStore[info+'_another'] = pagesite #For test
                        f.write("%s\t%s\t%s\t%s\t%s\n" % (ncp[0], ncp[1], pagesite, info, value))
                    f.close()
                else :
...
def GetMainUrl(pagesite, ncp) : #Crawl Maintext pagelist <on this page>
    ...
    try:
        ...
        logger.info("[GetUrl_ERROR]:" + str(pagesite))
        return -1
...
def CheckEnd(pagesite) : #Check whether this url has nextPage
    ...
    try:
        ...
        logger.info("[CheckEnd_ERROR]:" + str(pagesite))
        return -1
    soup = BeautifulSoup(html_doc)
    #print pagesite,soup
...
if __name__ == '__main__' : #Main for Stock_code_loop
    try :
        dictStore = cPickle.load(open("dictData.pkl","rb"))
    except :
        logger.info("[Tips]: pkl_not_exist")
    ...
    while line:
        ...
        line = infile.readline()
    infile.close()
    for i in xrange(0,1) : # 2600 lines
        ret = 0
        page = 1
    ...
    cPickle.dump(dictStore,open("dictData.pkl","w"))
