#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# Created on 2015-08-31 13:24:52
# Project: ReportList_shSinaF
...
MainUrl = "http://search.10jqka.com.cn/search"

class Handler(BaseHandler):
    crawl_config = {
    }
    
    @every(minutes=15 * 24 * 60)
    def on_start(self):
        for pos in xrange(0,len(stocklist)) :
            self.crawl(MainUrl, callback=self.index_page, save={"code":pos}, age=24*60*60, auto_recrawl=True, params={"preParams": "","querytype":"", "bgid":"", "edate":"", "sdate":"", "ts":"1", "f":"1", "qs":"1", "tid":"report", "w":str(stocklist[pos][1]).replace("sh","") } )

    @config(age=2 * 30 * 24 * 60 * 60)
    def index_page(self, response):
        i = 0  # get href-attr pair by Index
        s = [] # store attr by Index
        pos = response.save["code"]
        for attr in response.doc('.s_r_box > .s_r_attr').items():
            s.append(attr.text())
        for each in response.doc('.s_r_box > .s_r_title > a.reportPreview').items():
            self.crawl(each.attr.href, callback=self.detail_page, save={"code": pos, "attr": s[i]} )
            i = i + 1
        for nxt in response.doc('.grayr > a').items():
            if nxt.text() == u"下一页":
    ...
    def detail_page(self, response):
        ...
        maintext = ""
        if response.doc('.clearfix div.kuaizhao_contant').text() != u"" :
            maintext = response.doc('.clearfix div.kuaizhao_contant').text()
        title = response.doc('.kuaizhaolefttitle > h2').text()
        
        pos = response.save["code"]
        attr = response.save["attr"]
        ...
        for info in attr.split('/') :
            info = info.strip()
            if info.startswith(u'20') :
                time = info[2:].strip()
            elif info.startswith(u'机构') :
                source = info[3:].strip()
            elif info.startswith(u'评级') :
                rank = info[3:].strip()
            elif info.startswith(u'分析师') :
                researcher = info[4:].strip()
            elif info.startswith(u'研报类型') :
                kind = info[5:].strip()
            elif info.startswith(u'页数') :
                page = info[3:].strip()
