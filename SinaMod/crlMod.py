...
class crlMod:
    ...
    def GetDesc(self, pagesite) :
        ...
        for retry in xrange(0,3):
            ...
            if html_doc != "NULL" :
                ...
                l.Notice("Remove %s for Desc_Crawl failed %s" % (str(pxy), str(pagesite).replace(self.UrlHead,"")))
        soup = BeautifulSoup(html_doc.read().decode('gbk'), "html.parser")
        for blk in soup.find_all('div') :
            if blk.get('class') and blk.get('class')[0]=="main" and blk.get('class')[1]=="clearfix" :
                for div in blk.find_all('div', 'blk_container') :
                    # Gain Maintext
                    if div.text != "":
                        value = div.get_text().replace('\t','').replace('\r','').replace('\b','').replace('"',"'")
    ...
    def GetContent(self, soup) :
        if soup :
            ...
            for link in __retlist: 
                ...
                dateinfo = str(link[0].split('/')[-2])
                key = dateinfo + str(link[1].split('\t')[0])
                ...
                if (not self.dictStore.has_key(key)) or self.dictStore[key] != pagesite :
                    if len(value) > 4:
                        self.crawllist.append( "%s\t%s\t%s\t%s" % (sourceName, str(link[1]), pagesite, value.strip() ) ) 
    ...
    def Getdoc(self, url, pxy) :
        ...
        try:
            ...
            l.Notice("Getdoc Failed for %s" % str(ex))
            return "NULL"
    ...
    def GetMainUrl(self, url, pxy) : 
        for retry in xrange(0,3):
            try :
                ...
                if self.proxylist:
                    self.crawllist = []
                    # return self.GetMainUrl(url, self.Getpxy()) May Empty the Pool
                    return -2 # Wait for 5 Min
            if soup :
