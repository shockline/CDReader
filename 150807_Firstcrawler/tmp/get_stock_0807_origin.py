...
def GetContent(soup) :
    for link in soup['data'] :
        ...
        #print "%s\t%s\t%s" % (link['title'], pagesite, value)
        print "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (link['secuFullCode'], link['secuName'], link['companyCode'], link['rate'], link['change'], link['sratingName'], link['insName'], link['author'], pagesite, link['title'], value)
...
if __name__ == '__main__' :
    ...
    for i in xrange(1,2) :
        ...
        #    continue
    """
    desc = GetDesc("http://vis.10jqka.com.cn/free/viewzy/index/seq/1310165/stockCode/300426")
    value = '\t'.join(desc)
    print value
    http://datainterface.eastmoney.com//EM_DataCenter/js.aspx?type=SR&sty=GGSR&js=var%20qjEEjLvR={%22data%22:[(x)],%22pages%22:%22(pc)%22,%22update%22:%22(ud)%22,%22count%22:%22(count)%22}&ps=50&p=279&mkt=0&stat=0&cmd=2&code=&rt=47941243
